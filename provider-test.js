...
Promise.all(providers.map(item => {
  const start = new Date()
  return fetch(item.provider_url).then(() => {
    const time = new Date() - start
    success.push({ name: item.provider_name, url: item.provider_url, time: time })
  }).catch(err => {
    const time = new Date() - start
    fail.push({ name: item.provider_name, url: item.provider_url, time: time, error: err.message })
  })
})).then(() => {
  console.log(success)
  console.log('===========================================================')
